/*使用 g++ 编译时 g++ main.cpp -lpthread -o main -l 是链接到某个动态库 -o 是给编译出来的可执行程序取名*/
#include<iostream>
#include<unistd.h>
#include<pthread.h>
...
void* callback(void* arg)
{
	...
	// tData t;
	// t.num=100;
	// t.age=20; 临时变量作为返回值 当线程回收后 当前线程栈区内存也被回收 所以这样不行  用 new 申请堆区内存或者定义成 static 也行或者使用全局变量
...
int main()
{
	...
	pthread_t tid;// 线程 id 类型 无符号长整形

	pthread_create(&tid, NULL, callback, &tdata1);//子线程创建函数

	std::cout << "主线程：" << pthread_self() << std::endl;//pthread_self 返回当前线程 id

	//pthread_exit(NULL);//当前线程退出 且不影响其他线程运行 括号中的参数是线程返回的数据

	void* ptr;
	pthread_join(tid, &ptr);//线程回收函数，等待 tid 的线程退出并回收其返回值，第二个参数是一个 void** 的二级指针，
